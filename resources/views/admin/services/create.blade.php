    ...
    <div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.services.store') }}" method="POST">
            ...
            <div class="p-8">
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required 
                           ...
                           placeholder="Ex: Consultation, Inscription...">
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="4" 
                              class="block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 bg-slate-50 placeholder-slate-400"
                              placeholder="Description optionnelle de ce qui est fait à ce service">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="bg-slate-50 px-8 py-4 flex items-center justify-end gap-4 border-t border-slate-100">
                ...
                <button type="submit" class="inline-flex justify-center rounded-xl bg-indigo-600 py-2.5 px-6 text-sm font-bold text-white shadow-sm hover:bg-indigo-700 transition focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Créer le service</button>
            </div>
        </form>
    </div>
</x-app-layout>
